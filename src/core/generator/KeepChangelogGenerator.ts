...
`;
   ...
   */
  static isMagicrGenerated(content: string): boolean {
  ...
  private async appendToMagicrChangelog(
    ...
    for (const entry of entries) {
      ...
      if (entry.version === 'Unreleased') {
        ...
        result = this.updateUnreleasedSection(result, entry);
   ...
   */
  private updateUnreleasedSection(content: string, newEntry: ChangelogEntry): string {
    // Create parser instance for this method
    const parser = new (require('./ChangelogParser.js').default)();
